...
for y in range (0, WRF_latLen):  
    ...
    for x in range (0, WRF_lonLen):
            ...
            for x_small in range(0, good_lon_len):         
                ...
                #determine weights (u & v) - get an array of poitns of the smaller domain
                    if distance <= 3 * sigma_HYCOM: 
                        weight_HYCOM = 0.2*(1 / (sigma_HYCOM * np.sqrt(2 * np.pi))) * np.exp(-(distance**2) / (2*sigma_HYCOM**2))
                    else: 
                        weight_HYCOM = 0 
        ...
        #0.2*HYCOM weights 
                    sum_product_u25HYCOM = sum_product_u25HYCOM + weight_HYCOM*meanU_25[y_small,x_small]
